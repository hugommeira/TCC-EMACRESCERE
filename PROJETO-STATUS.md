...
- `next.config.mjs` tem `typescript.ignoreBuildErrors: true` e `eslint.ignoreDuringBuilds: true`, com TODO do próprio time original: "~30 erros pré-existentes de TS... remover antes do go-live final". Há ~53 erros de `exactOptionalPropertyTypes` espalhados por ~19 arquivos (não relacionados a auth). Rodar `npm run typecheck` pra ver a lista.
...
| `RESEND_API_KEY` | ✅ configurada e testada (envio real confirmado) — **em modo sandbox, só entrega pro e-mail dono da conta Resend** até verificar um domínio próprio |
