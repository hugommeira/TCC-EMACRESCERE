...
import { listPatientConsultations } from "@/services/api/consultation";
import {
...
export default async function PatientConsultationsPage({
  ...
  const { data: items, pages, total } = await listPatientConsultations(
    session.user.id,
    {
      page,
      limit: 10,
    ...
    <DashboardShell>
      ...
      {items.length === 0 ? (
        <EmptyState
          ...
          description={q || status ? "Ajuste os filtros ou solicite uma nova consulta." : "Você ainda não tem histórico."}
        />
      ) : (
        <SectionCard title="Suas consultas" className="overflow-hidden">
            ...
            <table className="w-full text-sm">
              <thead className="bg-slate-50">
                <tr>
                  ...
                  <th className="px-5 py-3 text-left text-[11px] font-semibold uppercase tracking-wider text-slate-500">Valor</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {items.map((c) => (
                  <tr key={c.id} className="transition-colors hover:bg-slate-50/40">
                    ...
                    <td data-label="Valor" className="px-5 py-3.5 text-slate-700">
